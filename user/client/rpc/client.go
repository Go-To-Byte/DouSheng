...
func NewCommentClientFromCfg() (*UserClient, error) {
	// 注册中心配置 [从配置文件中读取]
...
func NewCommentClientFromEnv() (*UserClient, error) {
	// 注册中心配置 [从环境变量文件中读取]
...
func newDefault(clientSet *client.ClientSet) *UserClient {
	conn := clientSet.Conn()
	return &UserClient{
...
func (c *UserClient) CommentService() user.UserClient {
	if c.Client == nil {
