...
import (
	...
	"github.com/Go-To-Byte/DouSheng/user/apps/user/impl/dal/query"
	"go.uber.org/zap"
)

func (s *UserServiceImpl) Add(user model.User) (err error) {
	q := query.Use(s.db)
	tx := q.Begin()
	defer func() {
...
func (s *UserServiceImpl) UserFindById(user model.User) []*model.User {
	q := query.Use(s.db)
	u := q.User
...
func (s *UserServiceImpl) UserFindByName(user model.User) []*model.User {
	q := query.Use(s.db)
	u := q.User
