...
func NewUserCenterClientFromCfg() (*CommentClient, error) {
	// 注册中心配置 [从配置文件中读取]
...
func NewUserCenterClientFromEnv() (*CommentClient, error) {
	// 注册中心配置 [从环境变量文件中读取]
...
func newDefault(clientSet *client.ClientSet) *CommentClient {
	...
	return &CommentClient{
		...
		// User 服务
		CommentClient: comment.NewCommentClient(conn),
	}
}

func (c *CommentClient) UserService() comment.CommentClient {
